// lib/caregiver_signup.dart
import 'dart:async';
import 'dart:typed_data';
import 'dart:io' show File; // only used on mobile, harmless on web
...
import 'package:image/image.dart' as img; // for real compression on web/mobile
...
class _CaregiverSignUpPageState extends State<CaregiverSignUpPage> {
  ...
  String _normalizeStorageUrl(String u) {
    final re = RegExp(r'(/v0/b/)([^/]+)\.firebasestorage\.app(/o/)');
    return u.replaceFirstMapped(re, (m) => '${m[1]}${m[2]}.appspot.com${m[3]}');
  }
  ...
  /// Explicitly use your project's bucket from firebase_options.dart
  // caregiver_signup.dart
  FirebaseStorage _storage() {
    // Uses elder-care-companion-app-0303.appspot.com from firebase_options.dart
    return FirebaseStorage.instance;
  }


  /// Upload a single image; returns downloadURL or null on failure.
  Future<String?> _uploadOne(
    ...
    try {
      debugPrint('[UPLOAD] start -> $folder / ${file.name}');
      final filename = '${DateTime.now().millisecondsSinceEpoch}_${file.name}';
      final ref = storage.ref('caregivers/$uid/$folder/$filename');

      final raw = await file.readAsBytes();
      final bytes = await _compress(raw);

      if (bytes.length > 5 * 1024 * 1024) {
        _toast('File too large (>5MB) after compression: ${file.name}');
        debugPrint('[UPLOAD] too-large ${bytes.length} bytes -> ${file.name}');
        return null;
      }
      ...
      final url = await snap.ref.getDownloadURL(); // already appspot.com
      debugPrint('[UPLOAD] success -> $url');
      return url;
    } on TimeoutException {
      _toast('Upload timed out for ${file.name}.');
      debugPrint('[UPLOAD] timeout -> ${file.name}');
      return null;
    } on FirebaseException catch (e, st) {
      // <- THIS is what we need if it fails
      _toast('Upload error for ${file.name}: ${e.code}');
      debugPrint('[UPLOAD] FirebaseException: code=${e.code} message=${e.message}');
      debugPrint('[UPLOAD] details: ${e.stackTrace ?? st}');
      return null;
    } catch (e, st) {
      _toast('Upload failed for ${file.name}');
  ...
  Future<List<String>> _uploadList(
    ...
    for (final f in files) {
      final u = await _uploadOne(storage, uid, folder, f);
      if (u != null) urls.add(u);
    }
    debugPrint('[UPLOAD] $folder saved ${urls.length}/${files.length}');
    return urls;
  }

  /// Tiny in-memory PNG uploader (debug). Call once to isolate rules/bucket issues.
  Future<void> _testTinyUpload(String uid) async {
    final s = _storage();
    final tiny = Uint8List.fromList([
      ...
      0x01,0x01,0x00,0x18,0xDD,0x8D,0xB1,0x00,0x00,0x00,0x00,0x49,0x45,0x4E,0x44,0xAE,0x42,0x60,0x82
    ]);
    try {
      final ref = s.ref('caregivers/$uid/debug/tiny.png');
      await ref.putData(tiny, SettableMetadata(contentType: 'image/png'));
      final url = await ref.getDownloadURL();
      debugPrint('[UPLOAD TEST] tiny uploaded: $url');
    } catch (e) {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // (optional) ensure currentUser is ready on some devices
      ...
      // (debug) isolate rules/bucket – uncomment if needed
      // await _testTinyUpload(uid);
      ...
      debugPrint('[UPLOAD] summary -> cert:${certUrls.length} dl:${dlUrls.length} ic:${icUrls.length} pic:${profileUrl != null}');
